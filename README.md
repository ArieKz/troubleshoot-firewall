...
I believed that it was the Linux firewall that was blocking the ping request from the Windows VM. ChatGPT suggested running ICMP Third check/ iptables directly (sudo iptables -L -n), with the intention to look for any rule that drops the ICMP. Then to temporarily (flush (diagnostic only)/  sudo iptables -F). I also ran (icmp tcpdump) showing that Linux was (listening on eth0). This did not resolve the issue, however, it did show that Linux was receiving the ping. 
